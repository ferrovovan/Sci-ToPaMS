...
def generate_grid(size, prob) -> list:
    """Генерирует квадратное поле с заданной вероятностью проходимости."""
    return [
                [1 if random.random() < prob else 0 for _ in range(size)]
                for _ in range(size)
        ]
...
def main():
    ...
    float_probabilities = [args.prob_start + step * args.prob_step for step in range(steps)]
